...
def get_data(input_path, fixed=None):
    ...
    t_ebs_lim, t_ebs_lim_150 = 100, 150
    df['fenia.input.heat.property.filling.source.limit'] = \
        (t_ebs_lim - t0) / (df['fenia.output.heat.temperature.ebs.max.value'] - t0) \
        * df['fenia.input.heat.property.filling.source.value']

    df['fenia.input.heat.property.filling.source.limit.per_container'] = \
        df['fenia.input.heat.property.filling.source.limit'] * df['fenia.output.heat.volume.filling'] \
        / df["gmsh.input.geometry.container.number"] * k

    df['fenia.input.heat.property.filling.source.limit.150'] = \
        (t_ebs_lim_150 - t0) / (df['fenia.output.heat.temperature.ebs.max.value'] - t0) \
        * df['fenia.input.heat.property.filling.source.value']

    df['fenia.input.heat.property.filling.source.limit.per_container.150'] = \
        df['fenia.input.heat.property.filling.source.limit.150'] * df['fenia.output.heat.volume.filling'] \
        / df["gmsh.input.geometry.container.number"] * k
...
def plot_t(input_path):
    ...
    df = df[df["gmsh.input.geometry.container.per_borehole"].isin([2, 5, 10])]
    df = df.melt(
        id_vars=[
            'gmsh.input.geometry.ebs.dh',
            'gmsh.input.geometry.ebs.dr',
        ...
        value_name="t")
    df = df.melt(
        id_vars=['zone', 't', 'gmsh.input.geometry.container.per_borehole'],
        value_vars=[
            'gmsh.input.geometry.ebs.dh',
            'gmsh.input.geometry.ebs.dr',
            'gmsh.input.geometry.borehole.dx',
    ...
    g = sns.lmplot(data=df,
                   ...
                   hue='zone',
                   row='variable',
                   col='gmsh.input.geometry.container.per_borehole',
                   # markers=['s', 'o'],
    ...
    g.set_titles(col_template="{col_name} containers", row_template="{row_name}")
    # g.tight_layout()
